...
class Pos extends CI_Controller
{
     ...
     */
     public function CloseRegister()
     {
         ...
         foreach ($sales as $sale) {
			...
			$cncld_invoices = Sale::find_by_sql("select sum(firstpayement) AS sum FROM sales where cancelled='1' AND cancelled_by='$this->register'");
			if($cncld_invoices[0]->sum > 0) {
				$cncld = $cncld_invoices[0]->sum;
			} else {
				$cncld = 0;
			}
                     $cash -= $cncld;
    ...
    {
			$this_shift = Register::find($this->register);
			$start = $this_shift->date;
			$end = $this_shift->closed_at;
				$cncld_invoices = Sale::find_by_sql("select sum(firstpayement) AS sum FROM sales where register_id='".$this->register."' ");
			if($cncld_invoices[0]->sum > 0) {
				$cncld = $cncld_invoices[0]->sum;
			} else {
				$cncld = 0;
