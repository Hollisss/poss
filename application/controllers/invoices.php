...
class Invoices extends CI_Controller
{
    ...
    public function ajax_delete($id)
    {
        ...
        $invc = Sale_item::find('all', array(
            'conditions' => array(
                'sale_id = ? AND cancelled = ?',
                $id,
				'0'
            )
        ));

		foreach($invc as $item) {
			$qt1 = $item ? $item->qt : 0;
			...
			$quantity = $stock ? $stock->quantity : 0;
				...
				$this->db->query("update stocks set quantity='".$newquant."' where product_id='".$item->product_id."'");
		 }
		//updating invoice as cancelled not deleted 
        date_default_timezone_set($this->setting->timezone);
		$date_time = date("Y-m-d H:i:s");
		$this->invoice->update(array('id' => $id), array('cancelled' => 1, 'date_cancelled' => $date_time, 'cancelled_by' => $this->register ));
		$posales = $this->db->query("update sale_items set cancelled='1', date_cancelled='".$date_time."', cancelled_by='".$this->register."' where sale_id='".$id."'");
