          ...
          <tbody class="itemslist">';
           foreach ($stores as $store) {
             ...
             $price = $stock ? ($stock->price ? $stock->price : '0') : '0';
               $content .= '<tr>
                 <td>'.$store->name.'</td>
                 <td><input type="number" id="quantity" store-id="'.$store->id.'" value="'.$quantity.'"></td>
                 <td><input type="number" id="pricestr" store-id="'.$store->id.'" value="'.$price.'"></td>
             </tr>';
           }
        $content .= '  </tbody>
       </table></div>';
    ...
    public function Viewproduct($id)
    {
      ...
      $content = '<div class="col-md-6"><div class="media">
         ...
         <div class="media-body">
          ...
          <b>'.label("Category").' :</b> '.$product->category.' <br>
          <b>'.label("Cost").' :</b> '.$product->cost.' '.$this->setting->currency.'<br>
          <b>'.label("ProductTax").' :</b> '.$product->tax.' <br>
          <b>'.label("Price").' :</b> '.$product->price.' '.$this->setting->currency.' <br>
          <b>'.label("Suppliers").' :</b> '.$product->supplier.' <br>
          <b>'.label("ProductDescription").' :</b> '.$product->description.'
         </div>
      </div></div>
...
*/
   ...
   public function barcode($row, $num, $productBcode,$product_ID)
   {
      $content = "";
      $bcs = 'code128';
      $height = 30;
      $width = 2;
      $product = Product::find_by_sql("select * from products where id='".$product_ID."' limit 1");
      for ($i=0; $i < $num; $i++) {
         $content .= '<div class="" style=" position: fixed;width: 100%;height: 100%;left: 0;top: 0;z-index: 10;line-height: 1;min-height: 100%;position: relative; text-align: center;box-sizing: border-box;border: solid black 0px">
          <div style="font-size: 13px;font-stretch: expanded">'.$product[0]->name.'</div>
          <b>'.label("price").' :</b>'.$product[0]->price.' '.$this->setting->currency.' <br>
      <img src="' . site_url('productcontroller/GenerateBarcode/' . sprintf("%05d", $productBcode) . '/' . $bcs . '/' . $height . '/' . $width) . '" alt="'.$productBcode.'" />
    </div>';
      }
