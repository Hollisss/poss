...
class Reports extends CI_Controller
{
    ...
    public function getProductReport()
    {
        ...
        if($product_id =="all") {
			$prducts = Sale_item::find_by_sql("SELECT * FROM `sale_items` WHERE cancelled='0' AND date between '$start' AND '$end' ORDER BY date");
        } else {
			$prducts = Sale_item::find_by_sql("SELECT * FROM `sale_items` WHERE cancelled='0' AND product_id = '$product_id' AND date between '$start' AND '$end' ORDER BY date");
		}
			if($product_id =="all") {
				$result = '<table id="Table" class="table table-striped table-bordered" cellspacing="0" width="100%"><thead><tr><th>' . label("ProductName") . '</th><th>' . label("Profit") . '</th></tr></thead><tbody>';
			} else {
        ...
        foreach ($prducts as $prd) {
			...
			if($product_id =="all") {
				$result .= '<tr><td>' . $prd->name . '</td><td>' . number_format((float)$profit, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td></tr>';
			} else {
        ...
        $totalallprofit = 0;
        /*$allprducts = Sale_item::find_by_sql("SELECT * FROM `sale_items` WHERE cancelled='0' AND date between '$start' AND '$end' ORDER BY date");

        foreach ($allprducts as $prd) {
			$prduct = Product::find($prd->product_id);
            $tax = $prd->subtotal * intval(substr($prduct->tax, 0, - 1)) / 100;
            $profit = $prd->subtotal - $tax - ($prduct->cost * $prd->qt);
		...
		*/
        $result .= '</tbody></table>';
        $result .= '<h1>' . label("TotalProfit") . ' : <span class="ReportTotal">' . $totalprofit . ' ' . $this->setting->currency . '</span></h1>';
