...
class Stats extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $TodayExpences = Expence::find_by_sql("select sum(amount) AS sum FROM expences where date = '$date'");
        $Todayreceiptvoucher = Receiptvoucher::find_by_sql("select sum(amount) AS sum FROM receiptvouchers where date = '$date'");
        $Top5product = Sale_item::find_by_sql("select name, product_id, sum(qt) AS totalquantity FROM sale_items where cancelled='0' AND DATE_FORMAT(date, '%Y') = $year GROUP BY product_id ORDER BY SUM(qt) DESC LIMIT 5");
        ...
        $this->view_data['Todayreceiptvoucher'] = number_format((float)$Todayreceiptvoucher[0]->sum, $this->setting->decimals, '.', '');
        $TodayProfit = 0;
        $allprducts = Sale_item::find_by_sql("SELECT * FROM `sale_items` WHERE cancelled='0' AND date='$date' ORDER BY date");
        ...
        $this->view_data['TodayProfit'] = number_format((float)$TodayProfit, $this->setting->decimals, '.', '');
