...
class Invoices extends CI_Controller
{
    ...
    {
        $list = $this->invoice->get_datatables(0);
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->invoice->count_all(0),
            "recordsFiltered" => $this->invoice->count_filtered(0),
            "data" => $data
        );
    ...
    public function ajax_delete($id)
    {
          ...
          $invc = Sale_item::find($id);
          $qt1 = $invc ? $invc->qt : 0;
          $product_id = $invc->product_id;
		  
          $stock = Stock::find('first', array('conditions' => array('product_id = ? ', $product_id)));
          $quantity = $stock ? $stock->quantity : 0;
			$newquant = $qt1+$quantity;
             $data = array(
                 "quantity" => $newquant
             );
					$this->db->query("update stocks set quantity='".$newquant."' where product_id='".$product_id."'");
		//updating invoice as cancelled not deleted
		$this->invoice->update(array('id' => $id), array('cancelled' => 1, 'date_cancelled' => date('Y-m-d') ));
		$posales = $this->db->query("update sale_items set cancelled='1', date_cancelled='".date('Y-m-d')."' where sale_id='".$id."'");
    ...
    public function ShowTicket($id)
    {
        ...
        foreach ($posales as $posale) {
			...
			<td style="text-align:right; width:70px; ">' . number_format((float)($posale->qt * $posale->price), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</td>
			</tr>';
            $i ++;
        }
