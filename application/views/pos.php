      ...
      <div class="col-md-7 right-side nopadding">
         <div class="row row-horizon">
                  ...
                  <?php foreach ($categories as $category):?>
                     <span class="categories cat_tabs" id="<?=$category->name;?>" style="display: none"><?=$category->name;?></span>
                  <?php endforeach;?>
               </div>
               <div class="col-sm-12">
                  <div id="searchContaner">
                      <div class="input-group stylish-input-group">
                          <input type="text" id="searchProd" onkeyup="$('#productList2').show();" class="form-control"  placeholder="<?=label('Search');?>" >
                          <span class="input-group-addon">
                              <button type="submit">
                                  <span class="glyphicon glyphicon-search"></span>
                              </button>
                          </span>
                          <span class="input-group-addon" id="toggleProdBtn" title="Show Products">
                              <button type="button">
                                  <span class="glyphicon glyphicon-eye-open"></span>
                              </button>
                          </span>
                          <span class="input-group-addon" id="toggleProdBtn2" title="Hide Products" style="display: none">
                              <button type="button">
						  ...
						  $(document).ready(function() {
							  $('#toggleProdBtn').click(function() {
								  $('#productList2').toggle();
								  $('.cat_tabs').toggle();
								  $('#toggleProdBtn').toggle();
								  $('#toggleProdBtn2').toggle();
							  });
							  $('#toggleProdBtn2').click(function() {
								  $('#productList2').toggle();
								  $('.cat_tabs').toggle();
								  $('#toggleProdBtn').toggle();
								  $('#toggleProdBtn2').toggle();
                        ...
                        <div class="product <?=$product->color;?> flat-box">
                           ...
                           <div class="mask">
                              <h3><?=number_format((float)$product->price, $this->setting->decimals, '.', '');?> <?=$this->setting->currency;?></h3>
								<?php
								$stock = Stock::find('first', array('conditions' => array('product_id = ?', $product->id)));
								$quantity = $stock ? $stock->quantity : '0';
								?>
                              <h3><?=label('Stock');?> : <?=$quantity;?></h3>
...
function add_posale(id)
{
   ...
   var number = $('.selectedHold').clone().children().remove().end().text();
     ...
     $.ajax({
         ...
         type: "POST",
         data: {name: name1, price: price1, product_id: id, number: number, registerid: <?=$this->register;?>},
         success: function(data)
         {
            if(data === 'stock'){
